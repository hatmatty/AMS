       ...
       Players.PlayerAdded.Connect((Player) => {
           Player.CharacterAdded.Connect((Char) => {
                ...
                Humanoid.Died.Connect(() => {
                    ...
                    for (const i = 0, i <= g.size(), i++) {
                        g[i].Parent = m
                    }
                    for (const v of g.GetDescendants()) {
                        if (v.IsA("BasePart")) {
                            v.SetNetworkOwner(Player)
                        } else if (v.IsA("Motor6D")) {
                            const [Att0, Att1] = [Instance.new("Attachment"), Instance.new("Attachment")]
                            Att0.CFrame = v.C0
                            Att1.CFrame = v.C1
                            Att0.Parent = v.Part0
                            Att1.Parent = v.Part1
                            const BSC = Instance.new("BallSocketConstraint")
                         ...
                         if (v.Name == "AccessoryWeld") {
                            const WC = Instance.new("WeldConstraint")
                            WC.Part0 = v.Part0
                            WC.Part1 = v.Part1
                            WC.Parent = v.Parent
                            v.Enabled = false
